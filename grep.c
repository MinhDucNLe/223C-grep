...
void search(const char* re) {  
   FILE *fp; int line = 0; int out = 0; char buf[GBSIZE];
   fp = fopen(file, "r");
   while(fgets(buf, GBSIZE, fp) != 0){
        if((strstr(buf, re)) != 0) { printf("%s", buf); out++; } line++; } exfile();
}
void readfile(const char* fn) {
  setnoaddr(); filename(fn);  init();
  addr2 = zero;  if ((io = open((const char*)file, 0)) < 0) { lastc = '\n';  error(file); }  setwide();  squeeze(0);
  ninbuf = 0;  append(getfile, addr2);  exfile();
}
int getch_(void) {  
